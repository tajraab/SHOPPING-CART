...
import React, { useState } from 'react';
 ...
 function Card() {
  ...
  const colorOptions = [
    ...
    { color: '#EAA523', img: 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/245657/t-shirt-large3.png' },
  ];
  ...
  const handleColorChange = (color, img) => {
    ...
    document.querySelectorAll('.size-value span.color').forEach(span => (span.style.color = color));
  ...
  return (
    <div className="container">
      <div className="image">
        <img src={colorOptions.find(option => option.color === selectedColor)?.img} alt="" />
      </div>
        ...
        <div className="wrapper">
          ...
          <div className="content size">
            ...
            <div className="size-value">
              <span className={`color ${selectedSize === 'XS' ? 'active' : ''}`} onClick={() => handleSizeChange('XS')}>
                XS
              </span>
              <span className={`color ${selectedSize === 'S' ? 'active' : ''}`} onClick={() => handleSizeChange('S')}>
                S
              </span>
              <span className={`color ${selectedSize === 'M' ? 'active' : ''}`} onClick={() => handleSizeChange('M')}>
                M
              </span>
              <span className={`color ${selectedSize === 'L' ? 'active' : ''}`} onClick={() => handleSizeChange('L')}>
                L
              </span>
              <span className={`color ${selectedSize === 'XL' ? 'active' : ''}`} onClick={() => handleSizeChange('XL')}>
                XL
              </span>
            </div>
          </div>
          <div className="content colour">
            ...
            <div className="colour-value">
              {colorOptions.map(option => (
                <span
                  key={option.color}
                  className={`${option.color} ${selectedColor === option.color ? 'active' : ''}`}
